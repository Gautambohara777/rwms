...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($waste_type) || !is_numeric($rate_per_kg)) {
        ...
        if ($check->num_rows > 0) {
            $error = "This waste type already exists. Addition cancelled.";
        } else {
            ...
            <?php if (!empty($wasteData)): ?>
                <?php foreach ($wasteData as $waste): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($waste['rate_per_kg']); ?></td>
                        <td><?php echo htmlspecialchars($waste['updated_at']); ?></td>
                        <td>
                            <a href="update_waste.php?id=<?php echo $waste['id']; ?>" class="update-btn">Update</a>
                            <a href="?delete_id=<?php echo $waste['id']; ?>" class="delete-btn" onclick="return confirm('Are you sure to delete this waste type?')">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
