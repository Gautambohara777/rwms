...
// fetch assigned pickups (collected)
$select_sql = "
    ...
    WHERE pr.`{$assignee_col}` = ?
      AND (LOWER(COALESCE(pr.status,'')) NOT IN ('completed','cancelled','Approved','In Progress','Refused'))
...
<div class="page">
    ...
    <?php if (count($rows) === 0): ?>
        <div class="no-data">No pending Approval</div>
    <?php else: ?>
